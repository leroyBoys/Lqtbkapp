...
gulp.task('cleantmp', del.bind(null, ['dist/tmp']))
/**
 * Lint source code
 */
gulp.task('lint', () => {
// return gulp.src(['*.{js,json}', '**/*.{js,json}', '!node_modules/**', '!dist/**', '!**/bluebird.js'])
return gulp.src(['*.{json}', '**/*.{json}', '!node_modules/**', '!dist/**', '!**/bluebird.js'])
    .pipe(plugins.eslint())
 ...
 * Build
 */
gulp.task('build', ['lint'], next => runSequence(['compile', 'extras'], 'cleantmp'))

gulp.task('watchbuild', ['lint'], next => runSequence(['compile', 'extras'],next))
/**
 * Watch source change
 */
